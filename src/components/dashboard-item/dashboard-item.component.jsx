...
import DashboardItemComment from '../dashboard-item-comment/dashboard-item-comment.component';
import DashboardItemImage from '../dashboard-item-img/dashboard-item-img.component';
...
const DashboardItem = ({username, avtuser, imgItem, userComment,ownerDes, likes}) => {
	const settings = {
		infinite: true,
		arrows: false,
		dots: true,
	...
	return(
		<div className = 'dashboard-block'>
			<div className='head-dashboard'>
				<span className='avtuser'><img src ={avtuser} alt='avtuser'/></span>
			...
			<div className='list-item-img'>
				<Slider {...settings}>
					{imgItem.map((listimg => 
						<DashboardItemImage
						key = {listimg.id}
						listImgItem = {listimg.listImgItem}
						/>
					))}
				</Slider>
			</div>
			...
			<div className='owner-des'>
				...
				<p className='item-des'><span>{username}</span> {ownerDes}</p>
			</div>
			<div className='count-comment'>
				<p>View all 6 comments</p>
			</div>
			<div className ='usercoment-box'>
				{userComment.map((useritem => 
					<DashboardItemComment
					key = {useritem.id}
					userCommentName = {useritem.userCommentName}
					userCommentDes = {useritem.userCommentDes}
					/>
				))}
			</div>
			
			<div className='add-commnet'>
				<button type='button'>
